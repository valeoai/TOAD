...
# Per-section inference profiler. Compare a ``use_cem=False`` baseline run
# against a ``use_cem=True`` run on the same data — the difference in the
# ``total`` row is the extra time introduced by the scorer + CEM refinement.
_PROFILER = InferenceTimer("drivor", log_every=50)
import logging
...
class DrivoRModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        bs = features["ego_status"].shape[0]
        with _PROFILER.section("total", batch_size=bs):
            output = self._forward_impl(features, bs)
        _PROFILER.increment()
        return output

    def _forward_impl(self, features: Dict[str, torch.Tensor], bs: int) -> Dict[str, torch.Tensor]:
        ...
        if self.num_cams > 0:
            ...
            if "image" in features :
                ...
                raise ValueError
            with _PROFILER.section("backbone", batch_size=bs):
        ...
        if (not self.training) and getattr(self._config, "use_cem", True):
            with _PROFILER.section("cem", batch_size=bs):
    ...
    def cem_refine(
        ...
        alpha = getattr(cfg, "cem_alpha", 1.0)
        ...
        for _ in range(num_iterations):
            eps = torch.randn(B, num_samples, P * 2, device=device, dtype=dtype)
            ...
            anchor_cost = ((ctrl_flat - anchor_flat.unsqueeze(1)) ** 2).mean(-1)
            score = (
                scorer_score
