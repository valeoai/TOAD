...
class DrivoRAgent(AbstractAgent):
    ...
    def initialize(self) -> None:
        ...
        if self._checkpoint_path != "":
            ...
            # Side-loaded MPC scorers (GTRS / SparseDrive) get their weights
            # from their own checkpoints in DrivoRModel.__init__, so they are
            # legitimately absent from the DrivoR checkpoint. Stay strict for
            # everything else.
            side_loaded = (
                "_drivor_model._gtrs_model.",
                "_drivor_model._sparsedrive_model.",
            )
            real_missing = [k for k in missing if not k.startswith(side_loaded)]
            if real_missing or unexpected:
                raise RuntimeError(
                    f"Error(s) in loading state_dict for {self.__class__.__name__}: "
                    f"missing keys {real_missing}, unexpected keys {list(unexpected)}"
                )
    ...
    def get_training_callbacks(self):
        ...
        # log_weight_decay is omitted: it only exists in PL >= 2.1 (False is
        # the default there anyway), and the rap env pins PL 2.0.
