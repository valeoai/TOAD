...
class DrivoRFeatureBuilder(AbstractFeatureBuilder):
    def __init__(self, config: Dict):
        ...
        # Optional side-loaded scorer configs (GTRS / SparseDrive used as the
        # MPC objective or rescoring baseline). Mirrors PadFeatureBuilder.
        self._gtrs_cfg = None
        if getattr(config, "use_gtrs_score", False) or getattr(
            config, "use_gtrs_rescore", False
        ):
            from navsim.agents.utils.gtrs_mpc import load_gtrs_cfg
            self._gtrs_cfg = load_gtrs_cfg(
                config.gtrs_config_path,
                getattr(config, "gtrs_overrides", None),
            )

        self._sparsedrive_cfg = None
        if getattr(config, "use_sparsedrive_score", False) or getattr(
            config, "use_sparsedrive_rescore", False
        ):
            from navsim.agents.utils.sparsedrive_mpc import load_sparsedrive_cfg
            self._sparsedrive_cfg = load_sparsedrive_cfg(
                config.sparsedrive_config_path,
                getattr(config, "sparsedrive_overrides", None),
            )
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        if self._gtrs_cfg is not None:
            from navsim.agents.utils.gtrs_mpc import (
                build_gtrs_camera_feature,
                build_gtrs_camera_feature_back,
                build_gtrs_status_feature,
            )
            features["gtrs_camera_feature"] = build_gtrs_camera_feature(
                agent_input, self._gtrs_cfg
            )
            if getattr(self._gtrs_cfg, "use_back_view", False):
                features["gtrs_camera_feature_back"] = build_gtrs_camera_feature_back(
                    agent_input, self._gtrs_cfg
                )
            features["gtrs_status_feature"] = build_gtrs_status_feature(
                agent_input, self._gtrs_cfg
            )

        if self._sparsedrive_cfg is not None:
            from navsim.agents.utils.sparsedrive_mpc import (
                build_sparsedrive_camera_feature,
                build_sparsedrive_status_feature,
            )
            features["sparsedrive_camera_feature"] = build_sparsedrive_camera_feature(
                agent_input, self._sparsedrive_cfg
            )
            features["sparsedrive_status_feature"] = build_sparsedrive_status_feature(
                agent_input, self._sparsedrive_cfg
            )
