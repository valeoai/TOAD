...
class Scene:
    ...
    def get_history_trajectory(self, num_trajectory_frames: Optional[int] = None) -> Trajectory:
        ...
        origin = StateSE2(*global_ego_poses[self.scene_metadata.num_history_frames-1])
    ...
    def get_total_trajectory(self, num_trajectory_frames: Optional[int] = None) -> Trajectory:
        """
        History + Future
        :return: trajectory dataclass
        """
        ...
        global_ego_poses = []
        for frame_idx in range(start_frame_idx, start_frame_idx + num_trajectory_frames + self.scene_metadata.num_history_frames):
            global_ego_poses.append(self.frames[frame_idx].ego_status.ego_pose)
