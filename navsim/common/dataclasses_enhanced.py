...
class AgentInput:
    ...
    lidars: List[Lidar]
    all_history_tokens: List
    all_future_tokens: List
    ...
    def from_scene_dict_list(
        ...
        all_history_tokens=[scene_dict_list[i]["token"] for i in range(num_history_frames)],
        all_future_tokens=[scene_dict_list[i+num_history_frames]["token"] for i in range(8)],

        return AgentInput(ego_statuses, cameras, lidars, all_history_tokens, all_future_tokens)
...
class SceneMetadata:
    ...
    all_history_tokens: list = None
    all_future_tokens: list = None
...
class Scene:
    ...
    def get_agent_input(self) -> AgentInput:
        ...
        cameras: List[Cameras] = []
        lidars: List[Lidar] = []
        future_tokens = self.scene_metadata.all_future_tokens
        history_tokens = self.scene_metadata.all_history_tokens
        ...
        return AgentInput(ego_statuses, cameras, lidars, history_tokens, future_tokens)
    ...
    def from_scene_dict_list(
        ...
        """
        assert len(scene_dict_list) >= 0, "Scene list is empty!"
        # print("len(scene_dict_list) ", len(scene_dict_list))
        scene_metadata = SceneMetadata(
    ...
    def load_from_disk(
        ...
        scene_metadata.all_history_tokens = []
        scene_metadata.all_future_tokens= []


        # print('len(scene_data["frames"])', len(scene_data["frames"]))
        for frame_idx, frame_data in enumerate(scene_data["frames"]):
            if frame_idx in list(range(4)):
                scene_metadata.all_history_tokens.append(frame_data["token"])
            elif frame_idx in list(range(4, 12)):
                scene_metadata.all_future_tokens.append(frame_data["token"])
        count = 500000
        if len(scene_metadata.all_future_tokens) < 8:
                last_token = scene_metadata.scene_token

                while len(scene_metadata.all_future_tokens) < 8:
                    scene_metadata.all_future_tokens.append(last_token + '_' + f'{int(scene_data["frames"][-1]["timestamp"]*0.001)*1000+count}')
                    count +=500000
        ...
        #     print('frame_data["timestamp"]', frame_data["timestamp"])
        # assert False
