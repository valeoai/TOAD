...
class MetricCacheProcessor:
    ...
    def __init__(
        ...
        print(f"Proposal sampling: num_poses={proposal_sampling.num_poses}, interval_length={proposal_sampling.interval_length}")
        print(f"Future sampling: num_poses={future_sampling.num_poses}, interval_length={future_sampling.interval_length}")
    ...
    def _extract_lane_keeping_speedup(self)-> Tuple[List[NDArray], KDTree]:
        """
        coords: (N, 2) array of centerline vertices in world frame.
        Returns dict of arrays needed for fast distance queries.
        """
        coords = np.asarray(self._pdm_closed._centerline.linestring.coords, dtype=np.float32)

        seg_a = coords[:-1]
        seg_b = coords[1:]
        seg_ab = seg_b - seg_a
        seg_len2 = np.einsum("ij,ij->i", seg_ab, seg_ab)
        mids = (seg_a + seg_b) * 0.5
    ...
    def compute_metric_cache(self, scenario: NavSimScenario) -> MetricCache:
        ...
        linestring_segments = self._extract_lane_keeping_speedup()
        ...
        return MetricCache(
            ...
            linestring_segments=linestring_segments,
        )
