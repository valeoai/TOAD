...
from navsim.planning.metric_caching.train_cache_processor_synt import MetricCacheProcessor
    ...
    def cache_scenarios_internal(args: List[Dict[str, Union[Path, DictConfig]]]) -> List[CacheResult]:
        ...
        def cache_single_scenario(
            ...
            scene = Scene.from_scene_dict_list(
                scene_dict,
                None,
                num_history_frames=cfg.train_test_split_synthetic.scene_filter.num_history_frames,
                num_future_frames=cfg.train_test_split_synthetic.scene_filter.num_future_frames,
        ...
        scene_filter: SceneFilter = instantiate(cfg.train_test_split_synthetic.scene_filter)
...
def cache_data(cfg: DictConfig, worker: WorkerPool) -> None:
    ...
    scene_loader = SceneLoader(
        sensor_blobs_path=None,
        data_path=Path(cfg.navsim_log_path),
        scene_filter=instantiate(cfg.train_test_split_synthetic.scene_filter),
        sensor_config=SensorConfig.build_no_sensors(),
    )
