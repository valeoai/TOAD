...
from navsim.evaluate.pdm_score_after_fix import pdm_score
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    ...
    # print("scene_loader_tokens_stage_two ", scene_loader_tokens_stage_two)
    # print("len(metric_cache_loader.tokens) ", len(metric_cache_loader.tokens))
...
def main(cfg: DictConfig) -> None:
    ...
    if combined:
        raise ValueError(
            'combined_inference is no longer supported: AgentLightningModule only serves the DrivoR predict path.'
        )
    ...
    if bool(cfg.get('save_predictions_and_exit', False)):
        logger.info(f"save_predictions_and_exit=true; skipping PDM scoring. Pkl at: {dump_path}")
        return

    # Ray workers only read model_trajectory[token]['trajectory']. The full
    # per-token payload (vocab-sized imi/subscore arrays, ~0.5 MB/token) is
    # already persisted to dump_path above; shipping it into every ray chunk
    # multiplies ~GBs across the object store and worker heaps and can
    # host-RAM OOM the job, so pass a trajectory-only view instead.
    slim_predictions = {
        token: {"trajectory": pred["trajectory"]}
        for token, pred in merged_predictions.items()
    }
    del predictions, all_predictions, merged_predictions
    ...
    data_points = [
        {
            ...
            "log_file": log_file,
            "tokens": tokens_list,
            "model_trajectory": slim_predictions
        }
    ...
    summary_last_row = pdm_score_df[pdm_score_df["token"] == "extended_pdm_score_combined"].to_dict()

    try:
        new_summary = {}

        for k,v in summary_last_row.items():
            new_k = k
            for r_k, r_v in replacements.items():
                if r_k in k:
                    new_k = k.replace(r_k, r_v)
            new_v = v[5914]
            new_summary[new_k] = new_v
    except:
    ...
    pprint.pprint(new_summary, sort_dicts=False)

    # Persist the same pprint output to a per-CEM-sweep file so multiple
    # cem_samples/cem_num_iterations runs under the same experiment don't
    # overwrite each other.
    cem_samples = OmegaConf.select(cfg, "agent.config.cem_num_samples")
    cem_iter = OmegaConf.select(cfg, "agent.config.cem_num_iterations")
    cem_num_elites = OmegaConf.select(cfg, "agent.config.cem_num_elites")
    if cem_num_elites is None:
        cem_num_elites= max(cem_samples // 8, 1) if cem_samples is not None else None

    if cem_samples is not None and cem_iter is not None:
        out_dir = Path(os.getenv("NAVSIM_EXP_ROOT")) / cfg.experiment_name
        out_dir.mkdir(parents=True, exist_ok=True)
        out_file = out_dir / f"{cem_samples}_{cem_iter}_{cem_num_elites}.txt"
        print(f"!!!!!!!!!!!!!!!!!!!Saving pprint summary to {out_file}")
        with open(out_file, "w") as f:
            pprint.pprint(new_summary, stream=f, sort_dicts=False)
        logger.info(f"Saved pprint summary to {out_file}")
