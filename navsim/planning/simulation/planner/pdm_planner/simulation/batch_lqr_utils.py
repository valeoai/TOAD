...
def _fit_initial_velocity_and_acceleration_profile(
    ...
    intermediate_solution = batch_matmul(
        torch.linalg.inv(torch.from_numpy(batch_matmul(A_T, A) + jerk_penalty * batch_matmul(R_T, R)).float()).cpu().numpy(), A_T
    )
...
def _fit_initial_curvature_and_curvature_rate_profile(
    ...
    intermediate = batch_matmul(torch.linalg.inv(torch.from_numpy(batch_matmul(A_T, A) + Q).float()).cpu().numpy(), A_T)
