...
from navsim.common.dataclasses_enhanced import AgentInput
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], str]:
        ...
        if self._cache_path is not None:
            ...
            for builder in self._feature_builders:
                features.update(builder.compute_features(agent_input))
            # Targets are training-only, except for builders that opt in via
            # `compute_at_inference = True` (e.g. RasterScoreTargetBuilder,
            # whose GT rasters feed the pseudo-PDMS CEM objective at eval).
            for builder in self._target_builders:
                if self.is_training or getattr(builder, "compute_at_inference", False):
                    targets.update(builder.compute_targets(scene))
        
        if hasattr(self._feature_builders[0], 'pipeline'):
            features, targets, token = self._feature_builders[0].pipeline(features, targets, token, True)
